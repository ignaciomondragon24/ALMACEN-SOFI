...
Diseñador Visual de Carteles para listas de precios.
"""
import json
...
class SignTemplate(models.Model):
    ...
    template_type = models.CharField('Tipo de Cartel', max_length=20, choices=TEMPLATE_TYPES, default='simple')
    width_mm = models.PositiveIntegerField('Ancho (mm)', default=50)
    height_mm = models.PositiveIntegerField('Alto (mm)', default=40)
    layout_json = models.TextField('Configuración Visual', default='{}', blank=True)
    is_active = models.BooleanField('Activo', default=True)
    is_default = models.BooleanField('Por Defecto', default=False)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='sign_templates',
    )
    ...
    def __str__(self):
        return f'{self.name} ({self.get_template_type_display()}) - {self.width_mm}x{self.height_mm}mm'
    ...
    def get_default_dimensions(cls, template_type):
        dims = {'simple': (50, 40), 'promotional': (70, 50), 'bulk': (100, 70), 'weight': (100, 70)}
        return dims.get(template_type, (50, 40))
...
class SignBatch(models.Model):
    ...
    template = models.ForeignKey(SignTemplate, on_delete=models.CASCADE, related_name='batches')
    paper_size = models.CharField('Tamaño de Papel', max_length=10, choices=PAPER_SIZES, default='A4')
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, related_name='sign_batches',
    )
    created_at = models.DateTimeField('Creado', auto_now_add=True)
    ...
    def __str__(self):
        return f'Lote #{self.pk} - {self.template.name} ({self.created_at.strftime("%d/%m/%Y %H:%M")})'

    @property
    def total_signs(self):
        return sum(i.copies for i in self.items.all())

    def get_paper_dimensions(self):
        sizes = {'A4': (210, 297), 'letter': (216, 279), 'legal': (216, 356)}
...
class SignItem(models.Model):
    ...
    batch = models.ForeignKey(SignBatch, on_delete=models.CASCADE, related_name='items')
    product = models.ForeignKey(
        'stocks.Product', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='sign_items',
    )
    custom_name = models.CharField('Nombre Custom', max_length=200, blank=True)
    custom_price = models.DecimalField('Precio Custom', max_digits=10, decimal_places=2, null=True, blank=True)
    gramaje = models.CharField('Gramaje', max_length=50, blank=True)
    promo_quantity = models.PositiveIntegerField('Cantidad Promo', null=True, blank=True)
    promo_price = models.DecimalField('Precio Promo', max_digits=10, decimal_places=2, null=True, blank=True)
    package_type = models.CharField('Tipo Empaque', max_length=20, choices=PACKAGE_TYPES, blank=True)
    quantity_per_package = models.CharField('Contenido Empaque', max_length=50, blank=True)
    price_100g = models.DecimalField('Precio 100g', max_digits=10, decimal_places=2, null=True, blank=True)
    price_250g = models.DecimalField('Precio ¼kg', max_digits=10, decimal_places=2, null=True, blank=True)
    price_1kg = models.DecimalField('Precio 1kg', max_digits=10, decimal_places=2, null=True, blank=True)
    ...
    def display_price(self):
        if self.custom_price is not None:
            return str(self.custom_price)
        if self.product:
            return str(self.product.sale_price)
        return '0'
