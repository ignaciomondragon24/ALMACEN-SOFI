...
class MPPointService:
    ...
    def get_user_id(self):
        """Obtiene el user_id de las credenciales o lo consulta a la API."""
        if self.credentials.user_id:
            return self.credentials.user_id

        # Consultar a la API si no está guardado
    ...
    def create_qr_order(self, external_pos_id, amount, external_reference,
                        title="Venta CHE GOLOSO", items=None):
        """
        Crea una orden QR dinámica para cobrar con código QR.

        Docs: https://www.mercadopago.com.ar/developers/es/docs/qr-code/integration-configuration/qr-dynamic/integration

        Args:
            external_pos_id: ID externo del punto de venta
        ...
        Returns:
            tuple: (success, data) - data incluye 'qr_data' con el string del QR
        """
        user_id = self.get_user_id()
        if not user_id:
            return False, {"error": "No se pudo obtener el User ID de Mercado Pago"}

        # Convertir monto a float con 2 decimales
        total_amount = float(Decimal(str(amount)).quantize(Decimal("0.01")))

        # Construir items si no se proveen
        if not items:
            items = [{
                "title": title,
        ...
        payload = {
            ...
            "total_amount": total_amount,
            "items": items,
            "notification_url": None,  # Se configurará via webhook global
        }

        logger.info(
            f"Creating QR order: user_id={user_id}, pos={external_pos_id}, "
            f"amount={total_amount}, ref={external_reference}"
        )
    ...
    def delete_qr_order(self, external_pos_id):
        """
        Elimina la orden QR activa de un punto de venta.
        ...
        user_id = self.get_user_id()
        if not user_id:
            return False, {"error": "No se pudo obtener el User ID"}
    ...
    def create_pos(self, external_pos_id, name, store_id=None):
        """
        Crea un punto de venta (POS) en Mercado Pago para QR.

        Args:
            external_pos_id: ID externo único del POS
            name: Nombre del POS
            store_id: ID de la sucursal (opcional)
